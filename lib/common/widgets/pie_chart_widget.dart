...
class _PieChartWidgetState extends State<PieChartWidget> {
  ...
  List<AggregatedCategoryData> _aggregateData(List<DataItem> dataItems) {
    ...
    for (var item in dataItems) {
      ...
      aggregatedAmounts.update(categoryId as int, (value) => value + amount,
          ifAbsent: () => amount);
      categories.putIfAbsent(categoryId as int, () => item.category);
        }
  ...
  Widget build(BuildContext context) {
    return FutureBuilder<List<DataItem>>(
      ...
      builder: (context, snapshot) {
        ...
        final percentage = widget.type == 'Expense'
            ? _totalExpense
            : widget.type == 'Income'
            ? _totalIncome
            : _totalLoan;
